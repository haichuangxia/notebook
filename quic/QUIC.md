# 流
**流**是**QUIC**向上层应用提供的**轻量**,**有序**的数据流抽象.**流**可以通过**发送数据**创建,其他的数据流管理,如:终结,取消,管理流控制(managing flow control),只会增加少量的开销.
...
## 流中收发数据
应用发送的数据,背封装到帧中.通过**StreamId**和**Offset**字段来实现数据有序化.为了实现数据的有序到达,需要有一定的**缓冲区**来保存失序到达的字节.

若干一个数据以相同的**offset**发送了多次,则接收端只会保存**最近一次**接收到的数据.

**流**是**有序的****字节流**的抽象,对于QUIC来说,它看不见除了**流**之外的其他结构.
...
## 发送端有限状态机
发送端的有限状态机示意图为:![States for sending parts of streams](./picture/States_for_sending_parts_of_streams.png)
...
## 接收端有限状态机
接收端的有限状态机状态为：![接收端有限状态机](./picture/states_of_receiving.png)
...
# 流量控制
流量控制（flow control）主要的目的是防止发送方发送过快，接收方缓冲区溢出。
## 数据流控制
...
$
streamID \leq max_streams \times 4+first\_stream\_id\_of\_type
$
的流才可以被打开。
